  ...
  <div class="flex flex-col gap-2 w-full">
    ...
    <div class="flex flex-wrap items-center gap-x-6 gap-y-1 min-h-[24px]">
      <div v-if="legendData?.aktual && showHist" class="text-[12px] font-mono flex items-center">
        <span class="font-bold mr-1 text-foreground">Open</span><span class="mr-3">{{ formatNumber(legendData.aktual.open) }}</span>
        <span class="font-bold mr-1 text-foreground">High</span><span class="mr-3">{{ formatNumber(legendData.aktual.high) }}</span>
        <span class="font-bold mr-1 text-foreground">Low</span><span class="mr-3">{{ formatNumber(legendData.aktual.low) }}</span>
        <span class="font-bold mr-1 text-foreground">Close</span>
      ...
      <div v-if="legendData?.volume != null && showVolume" class="text-[12px] font-mono text-muted-foreground flex items-center">
        <span class="font-bold mr-1">Volume</span><span>{{ formatCompact(legendData.volume) }}</span>
      </div>
